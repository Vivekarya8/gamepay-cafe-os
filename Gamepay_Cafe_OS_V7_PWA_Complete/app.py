...
def init():
    ...
    if DATABASE_URL:
        ...
        """CREATE TABLE IF NOT EXISTS transactions(id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,ts TEXT,day TEXT,type TEXT,detail TEXT,mode TEXT,customer TEXT,amount DOUBLE PRECISION,cost DOUBLE PRECISION,user TEXT,status TEXT DEFAULT 'ACTIVE')""",
        """CREATE TABLE IF NOT EXISTS expenses(id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,ts TEXT,day TEXT,category TEXT,mode TEXT,amount DOUBLE PRECISION,note TEXT,user TEXT)""",
        """CREATE TABLE IF NOT EXISTS purchases(id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,ts TEXT,day TEXT,supplier TEXT,product TEXT,qty INTEGER,rate DOUBLE PRECISION,total DOUBLE PRECISION,mode TEXT,user TEXT)""",
        """CREATE TABLE IF NOT EXISTS audit(id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,ts TEXT,user TEXT,action TEXT,detail TEXT)""",
        ...
        """CREATE TABLE IF NOT EXISTS gaming_sessions(id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,station_id INTEGER,start_ts TEXT,end_ts TEXT,minutes INTEGER,price DOUBLE PRECISION,label TEXT,status TEXT DEFAULT 'RUNNING',user TEXT)""",
        """CREATE TABLE IF NOT EXISTS inventory_movements(id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,ts TEXT,product TEXT,kind TEXT,qty DOUBLE PRECISION,reason TEXT,user TEXT)""",
        ...
        """CREATE TABLE IF NOT EXISTS supplier_payments(id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,ts TEXT,supplier TEXT,amount DOUBLE PRECISION,mode TEXT,note TEXT,user TEXT)""",
...
def customer():
    ...
    if DATABASE_URL:
        ...
        c.execute("INSERT OR REPLACE INTO customers(name,mobile,credit_limit,due,approved) VALUES(?,?,?,COALESCE((SELECT due FROM customers WHERE name=?),0),1)",(name,mobile,limit,name));c.commit();c.close();audit("CUSTOMER","Approved "+request.form["name"]);return redirect("/dashboard")
@app.post("/recovery")
def recovery():
    if not logged():return redirect("/")
